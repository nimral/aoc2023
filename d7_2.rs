...
fn build_hand(s: &str, card_order: &HashMap<char, usize>) -> Hand {
    ...
    let card_str = x.0;
    let mut max_poker_hand_type = 0;
    for new_char in card_order.keys() {
        let cvs = card_str.replace("J", &new_char.to_string()).chars().map(|x| card_order[&x]).collect::<Vec<usize>>();
        ...
        max_poker_hand_type = max_poker_hand_type.max(poker_hand_type);
    }

    Hand {
        card_values: card_str.chars().map(|x| card_order[&x]).collect::<Vec<usize>>(),
        bid: x.1.parse().unwrap(),
        poker_hand_type: max_poker_hand_type,
    }
}
